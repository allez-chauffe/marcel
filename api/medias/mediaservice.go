...
func (m *Service) DeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := os.RemoveAll(filepath.Join(config.Config.API.GetMediasDir(), strconv.Itoa(media.ID))); err != nil {
		commons.WriteResponse(w, http.StatusInternalServerError, err.Error())
		return
	}
...
func copyNewInstanceOfPlugin(media *medias.Media, mp *medias.MediaPlugin, path string) error {
	return commons.CopyDir(filepath.Join(config.Config.API.GetPluginsDir(), mp.EltName, "frontend"), filepath.Join(path, "frontend"))
}

func getPluginDirectory(media *medias.Media, eltName string, instanceID string) string {
	return filepath.Join(config.Config.API.GetMediasDir(), strconv.Itoa(media.ID), eltName, instanceID)
}
